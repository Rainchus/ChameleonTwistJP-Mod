...
genMain:
	...
	$(file >> $(OUTPUT_FILE),.open "rom/ct1JP.z64", "rom/ct1JP.mod.z64", 0 // Open the ROM file)
	$(file >> $(OUTPUT_FILE),.org 0x9E7A80)
	$(file >> $(OUTPUT_FILE),.importobj "assets/TitleScreen.rgba16.png.o")
	$(file >> $(OUTPUT_FILE),.include "asm/ct1JP.asm" // Include ct1.asm to tell armips' linker where to find the game's function(s))
	$(file >> $(OUTPUT_FILE),.headersize 0x7F810000)
	$(file >> $(OUTPUT_FILE),.org 0x80420000)
